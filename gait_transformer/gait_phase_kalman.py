...
def measurements(x):
    ...
    return jnp.array([jnp.cos(omega), jnp.sin(omega),
           jnp.cos(omega + phi1), jnp.sin(omega + phi1),
           jnp.cos(omega + phi2), jnp.sin(omega + phi2),
           jnp.cos(omega + phi3), jnp.sin(omega + phi3)])
...
def gait_kalman_smoother(phases, dt=1.0/30.0, smoothing=True):
    ...
    if smoothing:
        backpass = {'a_priori_x': filtered['a_priori_x'][1:], 'a_priori_P': filtered['a_priori_P'][1:],
                    'a_posteriori_x': filtered['a_posteriori_x'][:-1], 'a_posteriori_P': filtered['a_posteriori_P'][:-1]}
